...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = np.sum(-np.log(probs[np.arange(N),y])) / N + 0.5 * reg * np.sum(W1*W1) + 0.5 * reg * np.sum(W2*W2) + 0.5 * reg * np.sum(b1*b1) + 0.5 * reg * np.sum(b2*b2)
        ...
        grads['W1'] = dW1 + dreg_W1
        grads['b1'] = db1
        grads['W2'] = dW2 + dreg_W2
        grads['b2'] = db2
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            batch_random = np.random.choice(num_train,batch_size)
    ...
    def predict(self, X):
        ...
        h1_tmp = X.dot(self.params['W1']) + self.params['b1']
        h1 = np.maximum(0,h1_tmp)
